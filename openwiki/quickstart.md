...
| Entry point | Registered target | Implementation module | Route changes here when you need to… |
|---|---|---|---|
| `agent` | `agent.graphs.agent:traced_agent` | `agent.server` | Change the main coding graph, per-thread sandbox lifecycle, tools, prompts, models, or middleware. |
| `reviewer` | `agent.graphs.reviewer:traced_reviewer_agent` | `agent.reviewer` | Change read-only PR review and finding behavior. |
| `analyzer` | `agent.graphs.analyzer:traced_analyzer` | `agent.analyzer` | Change repository review-style analysis. |
| `chat` | `agent.graphs.chat:traced_chat_agent` | `agent.chat` | Change dashboard PR chat, which uses seeded PR virtual files and read-only GitHub-backed repository access without a sandbox. |
| `scheduler` | `agent.graphs.scheduler:get_scheduler` | `agent.scheduler` | Change cron-triggered reconciliation, CI watch evaluation, background-task monitoring, session-cost refresh, or scheduled agent work. |
| HTTP app | `agent.webapp:app` | `agent.api.app` | Change dashboard, health, plan and workflow-approval APIs, or Slack, Linear, and GitHub webhook ingress. |
...
- `agent.webapp:app` is a compatibility re-export of the FastAPI application in `agent.api.app`. Startup pins one event loop and validates sandbox and local-development LLM configuration; shutdown closes cached models. Credentialed CORS is enabled only for configured dashboard origins and rejects `*`.
...
Use `make dev` for graph execution or an end-to-end webhook-to-run change. `make run` starts FastAPI only, not the LangGraph runtime. `make desktop` starts the Electron wrapper and expects its backend to be available. The project requires Python `>=3.14`; the served LangGraph runtime is Python 3.14. Python code is async-only by convention: add the async implementation, not parallel synchronous and asynchronous paths.
