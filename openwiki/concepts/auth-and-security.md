...
- Bot-token-only mode means `LANGSMITH_API_KEY` is set while both `X_SERVICE_AUTH_JWT_SECRET` and `USER_ID_API_KEY_MAP` are absent. Without those per-user resolution facilities, git operations use the installation token unless the mapped dashboard token is available.
